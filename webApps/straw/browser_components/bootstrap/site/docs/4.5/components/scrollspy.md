...
{% highlight js %} $('body').scrollspy({ target: '#navbar-example' })
{% endhighlight %}
...
the DOM like `<div id="home"></div>`. {% endcapture %} {% include callout.html content=callout type="danger" %}
...
and their corresponding nav items will never be highlighted. {% endcapture %} {% include callout.html content=callout
type="info" %}
...
{% highlight js %} $('[data-spy="scroll"]').each(function () { var $spy = $(this).scrollspy('refresh')
})
{% endhighlight %}
...
{% highlight js %} $('[data-spy="scroll"]').on('activate.bs.scrollspy', function () { // do something... })
{% endhighlight %}
